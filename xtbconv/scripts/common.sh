...
echo "INFO:Base Directory = \"${BASE_DIR}\"" >> ${LOG_FILE}
echo "INFO:Generated Directory = \"${GENERATED_DIR}\"" >> ${LOG_FILE}
echo "INFO:Temp Directory = \"${TEMP_DIR}\"" >> ${LOG_FILE}
echo "INFO:Converted list = \"${CONVERTED_LIST_FILE}\"" >> ${LOG_FILE}
echo "INFO:MkXTBWikiplexus = \"${MKXTBWIKIPLEXUS}\"" >> ${LOG_FILE}
echo "INFO:YomiGenesis = \"${YOMIGENESIS}\"" >> ${LOG_FILE}
echo "INFO:MkRax = \"${MKRAX}\"" >> ${LOG_FILE}
echo "INFO:MkXTBIndexDB = \"${MKXTBINDEXDB}\"" >> ${LOG_FILE}
echo "INFO:MkImageComplex = \"${MKIMAGECOMPLEX}\"" >> ${LOG_FILE}

if [ ! -e ${MKXTBWIKIPLEXUS} ]; then
  echo "ERROR:MkXTBWikiplexusが見つかりません。変換を中断しました。" >> ${LOG_FILE}
  exit 3
elif [ ! -e ${YOMIGENESIS} ]; then
  echo "ERROR:YomiGenesisが見つかりません。変換を中断しました。" >> ${LOG_FILE}
  exit 3
elif [ ! -e ${MKRAX} ]; then
  echo "ERROR:MkRaxが見つかりません。変換を中断しました。" >> ${LOG_FILE}
  exit 3
elif [ ! -e ${MKXTBINDEXDB} ]; then
  echo "ERROR:MkXTBIndexDBが見つかりません。変換を中断しました。" >> ${LOG_FILE}
  exit 3
elif [ ! -e ${MKIMAGECOMPLEX} ]; then
  echo "ERROR:MkImageComplexが見つかりません。変換を中断しました。" >> ${LOG_FILE}
  exit 3
fi
...
function upload_files(){
  if [ "${flag_upload}" = "true" ]; then
    echo "INFO:アップロードを開始します" >> ${LOG_FILE}
  ...
  if [ "${flag_delete}" = "true" ]; then
    ...
    do
      ...
      if [ -e ${GENERATED_DIR}/compression/${full_name}/${DATE}/${out_bundle_dirname}.7z ]; then
        ...
        echo "INFO:削除:${GENERATED_DIR}/compression/${full_name}/${DATE}/${out_bundle_dirname}.7z.001" >> ${LOG_FILE}
        cd ${GENERATED_DIR}/compression/${full_name}
        rm -rf ${DATE} 2>> ${LOG_FILE}
      else
        echo "NOTICE:${out_bundle_dirname}.7zは見つかりません。" >> ${LOG_FILE}
      fi
